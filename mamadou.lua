...
-- Fonction CreateStaffNotification supprimée car plus utilisée avec la détection manuelle
--[[
    ...
    for _, player in pairs(Players:GetPlayers()) do
        if player ~= LocalPlayer and player.Team then
            ...
            if teamName:find("staff") or teamName:find("admin") or teamName:find("moderator") or teamName:find("mod") then
                table.insert(staffDetected, {name = player.Name, team = player.Team.Name, userid = player.UserId})
                currentStaffs[player.Name] = true
            end
        end
    end

    -- Plus de notifications automatiques - seulement pour vérification manuelle

    -- Auto-quit si activé et qu'il y a des staffs
    if #staffDetected > 0 and AutoQuitEnabled then
...
-- Fonction simplifi\u00e9e pour marquer visuellement les staff
local function CreateSimpleStaffMarker(staffPlayer)
    ...
    warningLabel.Text = "⚠️ STAFF: " .. staffPlayer.Name
    ...
    -- Supprimer les marqueurs après 5 secondes
    task.spawn(function()
        task.wait(5)
    ...
    print("Prostone Hub - Marqueur créé sur le staff: " .. staffPlayer.Name)
end
...
local CheckStaffButton = ESPTab:CreateButton({
    ...
    Callback = function()
        local staffList = CheckForStaff()
        
        if #staffList > 0 then
            local detailedMessage = "🚨 STAFFS DÉTECTÉS 🚨\n\n"
            local shortList = ""
            
            for i, staff in ipairs(staffList) do
                detailedMessage = detailedMessage .. "👤 " .. staff.name .. " - " .. staff.team .. "\n"
                detailedMessage = detailedMessage .. "   ID: " .. staff.userid .. "\n\n"
                
                if i <= 3 then
                    shortList = shortList .. staff.name
                    if i < #staffList and i < 3 then
                        shortList = shortList .. ", "
                    elseif i == 3 and #staffList > 3 then
                        shortList = shortList .. " et " .. (#staffList - 3) .. " autre(s)"
                    end
                end
            end
            ...
            Rayfield:Notify({
                Title = "⚠️ " .. #staffList .. " Staff(s) Détecté(s)",
                Content = shortList,
                Duration = 8,
                Image = 4483362458,
            })
            
            -- Créer automatiquement des marqueurs visuels
            for _, staff in ipairs(staffList) do
                local player = Players:FindFirstChild(staff.name)
                if player and player.Character then
                    CreateSimpleStaffMarker(player)
                end
            end
            ...
            Rayfield:Notify({
                Title = "✅ Aucun Staff",
                Content = "La partie est sécurisée, aucun staff détecté.",
                Duration = 4,
                Image = 4483362458,
            })
        end
...
local VisualizeStaffButton = ESPTab:CreateButton({
    Name = "Marquer Staff visuellement",
    Callback = function()
        ...
        if staffFound then
            Rayfield:Notify({
                Title = "👁️ Marqueurs Staff",
                Content = "Staffs marqués pendant 5 secondes",
                Duration = 3,
                Image = 4483362458,
            })
        else
            Rayfield:Notify({
                Title = "✅ Aucun Staff",
                Content = "Aucun staff trouvé à marquer",
                Duration = 2,
                Image = 4483362458,
            })
        end
...
local AdonisButton = AntiCheatTab:CreateButton({
    Name = "Adonis Core Blocker",
    Callback = function()
        pcall(function()
            local totalBlocked = 0
            local adonisFound = false
            
            -- Mots-clés pour détecter Adonis (plus complets)
            local adonisKeywords = {
                "adonis", "admin", "anticheat", "antiexploit", "logs", "command", 
                "kick", "ban", "warn", "mute", "shutdown", "place", "teleport"
            }
            
            -- Bloquer les scripts Adonis
            local function blockAdonisScripts()
                local blocked = 0
                for _, service in pairs({game.ServerScriptService, game.ReplicatedStorage, game.StarterGui, game.StarterPlayer, workspace}) do
                    if service then
                        for _, obj in pairs(service:GetDescendants()) do
                            if obj:IsA("Script") or obj:IsA("LocalScript") or obj:IsA("ModuleScript") then
                                ...
                                pcall(function()
                                    if obj.Source then
                                        objSource = obj.Source:lower()
                                    end
                                end)
                                
                                -- Vérifier les mots-clés
                                local isAdonis = false
                                for _, keyword in pairs(adonisKeywords) do
                                    if objName:find(keyword) or objSource:find(keyword) then
                                        isAdonis = true
                                        adonisFound = true
                                        break
                                    end
                                ...
                                if isAdonis then
                                    pcall(function()
                                        ...
                                        obj:Destroy()
                                        blocked = blocked + 1
                                        print("Prostone Hub - Script bloqué: " .. obj.Name)
                                    end)
                                end
                            end
                        end
            ...
            -- Bloquer les RemoteEvents/Functions Adonis
            local function blockAdonisRemotes()
                local blocked = 0
                for _, service in pairs({game.ReplicatedStorage, workspace, game.StarterPlayer}) do
                    if service then
                        for _, obj in pairs(service:GetDescendants()) do
            ...
            -- Bloquer les dossiers et autres objets Adonis
            local function blockAdonisFolders()
                local blocked = 0
                for _, service in pairs({game.ReplicatedStorage, workspace, game.StarterPlayer, game.StarterGui}) do
                    if service then
                        for _, obj in pairs(service:GetDescendants()) do
                            if obj:IsA("Folder") or obj:IsA("Model") or obj:IsA("BindableEvent") or obj:IsA("BindableFunction") then
                                local objName = obj.Name:lower()
                                ...
                                if isAdonis then
                                    pcall(function()
                                        obj:Destroy()
                                        blocked = blocked + 1
                                        print("Prostone Hub - Objet bloqué: " .. obj.Name)
                                    end)
                                end
                            end
                        end
            ...
            -- Bloquer les outils spécifiques d'Adonis
            local function blockAdonisTools()
                local blocked = 0
                -- Chercher spécifiquement "Adonis_Loader" et "Adonis_Main"
                for _, service in pairs(game:GetChildren()) do
                    for _, obj in pairs(service:GetDescendants()) do
                        local objName = obj.Name:lower()
                        if objName == "adonis_loader" or objName == "adonis_main" or 
                           objName == "adonis" or objName:find("adonis_") then
                            pcall(function()
                                obj:Destroy()
                                blocked = blocked + 1
                                adonisFound = true
                                print("Prostone Hub - Adonis principal bloqué: " .. obj.Name)
                            end)
                        end
                    end
                end
            ...
            print("Prostone Hub - Début du blocage Adonis...")
            
            -- Exécuter tous les bloqueurs
            totalBlocked = totalBlocked + blockAdonisScripts()
            totalBlocked = totalBlocked + blockAdonisRemotes()
            totalBlocked = totalBlocked + blockAdonisFolders()
            totalBlocked = totalBlocked + blockAdonisTools()
            
            -- Surveillance continue améliorée
            spawn(function()
                while true do
                    blockAdonisScripts()
                    blockAdonisRemotes()
                    blockAdonisFolders()
                    blockAdonisTools()
                    wait(3) -- Vérification plus fréquente
                end
            end)
            
            print("Prostone Hub - Blocage terminé. Éléments bloqués: " .. totalBlocked)
            
            -- Afficher la notification selon le résultat
            if totalBlocked > 0 or adonisFound then
                Rayfield:Notify({
                    Title = "Adonis Core Blocker",
                    Content = "tkrl c bypass adonis",
                    Duration = 5,
                    Image = 4483362458,
                })
                print("Prostone Hub - ✅ Bypass Adonis réussi! " .. totalBlocked .. " éléments bloqués")
            else
                -- Même si rien n'est trouvé, on considère que c'est un succès (pas d'Adonis présent)
                Rayfield:Notify({
                    Title = "Adonis Core Blocker",
                    Content = "tkrl c bypass adonis",
                    Duration = 5,
                ...
                print("Prostone Hub - ✅ Aucun Adonis détecté, vous êtes protégé!")
            end
        end)
    end,
})
